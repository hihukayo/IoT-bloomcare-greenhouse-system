...
STM32F103 温室环境监测节点（采集与执行端）。DHT22（AM2302）采集温湿度，通过 USART2 上的
...
- **Components**：链路协议本体（组帧、CRC16/MODBUS、解析、ACK/NAK、重传、QUERY 应答、
  ...
  换 MCU 只需重新提供这四个函数。
- **Device**：`sen_dht22` 只管把 DHT22 读出来；`dev_manager` 维护传感器/执行器两张表，
...
```
上行采集   task_sensor -> Dev_Manager_Collect -> sen_dht22 -> GPIO
...
数据项（每项 5 字节：ID 1B + int32 4B，数值一律乘 100 传输）：

数值的缩放由驱动自己做，因为不同芯片的分辨率不同：DHT22 是 0.1 分辨率，
直接上报 2340 表示 23.40 C，负温保留符号；换成只有整数分辨率的芯片，
就在自己的驱动里乘 100。这样 Tasks 层不需要知道任何单位换算。
...
1. 在 `Device/Sensors/Src/` 或 `Device/Actuators/Src/` 写驱动，实现 `Dev_Sensor_t`
   或 `Dev_Actuator_t` 里的函数，并导出一个实例（参考 `sen_dht22.c` 末尾的 `g_sen_dht22`）。
2. 头文件放进对应的 `Inc/` 目录。
...
驱动的引脚只写 CubeMX 的标签，例如 `SEN_DHT22_GPIO_PIN = DHT22_Pin`，
端口时钟用 `SEN_DHT22_GPIO_CLK_ENABLE()`（内部走 `BSP_GPIO_ClkEnable()`）。
...
1. 引脚标签（如 `DHT22_Pin` / `DHT22_GPIO_Port`）是否还在，改名会导致 `sen_dht22.h` 编译报错。
